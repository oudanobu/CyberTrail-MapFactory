...
if [ ! -f "maps/admin_boundaries/zhenan.geojson" ]; then
    echo "Fetching Zhenan boundary from Overpass API..."
    curl -s -g -X POST -d '[out:json][timeout:60];relation["name"="振安区"]["admin_level"="8"];out geom;' https://overpass-api.de/api/interpreter > maps/admin_boundaries/zhenan.json
    echo "Converting to GeoJSON..."
    npx -y osmtogeojson maps/admin_boundaries/zhenan.json > maps/admin_boundaries/zhenan.geojson
fi

echo "Cropping Zhenan..."
